...
1. **Cloud Model Training (雲端模型訓練)**: 需要龐大的運算力與記憶體頻寬，以及強大的晶片互連能力 (Scale-out)。代表晶片：Nvidia H100 / B200 (Blackwell 架構), Google TPU v5p / Trillium (v6), AMD MI300X。
2. **Cloud Model Inference (雲端模型推論)**: 追求高吞吐量、低延遲與高性價比。代表晶片：Groq LPU, AWS Inferentia, Nvidia L40S, Google TPU v5e / Trillium (v6)。
