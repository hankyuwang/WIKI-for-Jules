...
## 專有名詞補充解說
- **Relay IR**：TVM 的高階中間表示形式，主要處理計算圖級別的抽象，例如神經網路中層與層的連接關係，負責硬體無關的最佳化。
- **TIR (Tensor IR)**：TVM 的低階中間表示形式，負責處理張量級別的具體計算邏輯，例如迴圈展開（Loop Unrolling）或是向量化，以便針對特定硬體架構產生最佳的組合語言。
- **BYOC (Bring Your Own Codegen)**：這是一個允許硬體製造商將他們自己專屬的編譯器或程式碼產生器整合到 TVM 框架中的機制，讓新的 NPU 可以快速得到上層框架的支援。
